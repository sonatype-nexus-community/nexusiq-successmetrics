...
public class QuarantinedComponents implements CsvFileService {
    ...
    static List<String[]> getQuarentinedComponentsFromData(JsonObject dataObject) {
        ...
        for (JsonObject result : results.getValuesAs(JsonObject.class)) {
            ...
            Boolean quarantined = result.isNull("quarantined") ? null : result.getBoolean("quarantined");
            ...
            if (quarantinePolicyViolations.isEmpty()) {
                data.add(createDataLine(repository, quarantineDate, dateCleared, displayName, format, quarantined, "None", "0", ""));
            } else {
                processQuarantinePolicyViolations(data, quarantinePolicyViolations, repository, quarantineDate, dateCleared, displayName, format, quarantined);
            }
        }
        return data;
    }

    private static void processQuarantinePolicyViolations(List<String[]> data, JsonArray quarantinePolicyViolations, String repository, String quarantineDate, String dateCleared, String displayName, String format, Boolean quarantined) {
        for (JsonObject quarantinePolicyViolation : quarantinePolicyViolations.getValuesAs(JsonObject.class)) {
            ...
            for (JsonObject constraintViolation : constraintViolations.getValuesAs(JsonObject.class)) {
                ...
                data.add(createDataLine(repository, quarantineDate, dateCleared, displayName, format, quarantined, policyName, String.valueOf(threatLevel), reason));
            }
        }
    }

    private static String[] createDataLine(String repository, String quarantineDate, String dateCleared, String displayName, String format, Boolean quarantined, String policyName, String threatLevel, String reason) {
        return new String[]{
                repository,
                quarantineDate,
                dateCleared,
