...
public class ParseReasons {
    ...
    private static String getCVE(JsonArray reasons) {
        Set<String> cves =
                reasons.getValuesAs(JsonObject.class).stream()
    ...
    public static String getLicense(JsonArray reasons) {
        Set<String> licenses =
                reasons.getValuesAs(JsonObject.class).stream()
                        ...
                        .peek(
                                licenseFound -> {
                                    if (!licenseFound.contains("(")
                                            || !licenseFound.contains(")")) {
                                        log.debug(
                                                "licenseFound string does not contain brackets: "
                                                        + licenseFound);
                                    }
                                })
                        .filter(
                                licenseFound ->
                                        licenseFound.contains("(") && licenseFound.contains(")"))
                        .map(
                                licenseFound ->
                                        licenseFound
                                                .substring(
                                                        licenseFound.indexOf("(") + 1,
                                                        licenseFound.indexOf(")"))
                                                .replace("'", ""))
                        .collect(Collectors.toSet());
